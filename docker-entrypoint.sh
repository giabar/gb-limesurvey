...
#ServerName in default-ssl.conf
sed -i "s|.*\bServerName\b.*|                ServerName $NEW_SERVER_NAME:443|" /etc/apache2/sites-available/default-ssl.conf
#SSLEngine on-off
sed -i "s|.*\bSSLEngine\b.*|                 SSLEngine $ENABLE_SSL|" /etc/apache2/sites-available/default-ssl.conf

if [ "$ENABLE_SSL" = "off" ]; then
  sed -i "s|.*\bSSLCertificateFile\b.*|                #SSLCertificateFile|" /etc/apache2/sites-available/default-ssl.conf
  sed -i "s|.*\bSSLCertificateKeyFile\b.*|                #SSLCertificateKeyFile|" /etc/apache2/sites-available/default-ssl.conf
fi
